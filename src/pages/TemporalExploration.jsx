...
function EventDetailPanel({ selectedEvent }) {
  return (
...
function DayView() {
  const [selectedDate, setSelectedDate] = useState(weekData[0].date)
  const [selectedEvent, setSelectedEvent] = useState(null)

  const dayRow = dayInfoForDate(selectedDate)
  const dayMetrics = daySleepMetrics(dayRow.segments)
  const dayActivityTotals = dayTotals(dayRow.segments)
  const dayAnomalies = anomalies.filter((a) => sameDate(a.date, selectedDate))
  const eventCount = dayRow.segments.filter((s) => s.type !== 'idle').length
  const dayWeekLabel = `Week ${isoWeekNumber(selectedDate)}`
  ...
  return (
    <>
      ...
      <div className="metric-row">
        <div className="metric-card">
          <div className="metric-label">Sleep</div>
          <div className="metric-value">{fmtDuration(dayMetrics.duration)}</div>
          <div className="metric-sub">
            <DeltaIndicator
              minutes={Math.round((dayMetrics.duration - avgDayMetrics.duration) * 60)}
              avg={`avg ${fmtDuration(avgDayMetrics.duration)}`}
            />
          </div>
        </div>
        <div className="metric-card">
          <div className="metric-label">Bedtime</div>
          <div className="metric-value">{fmtTime(dayMetrics.bedtime)}</div>
          <div className="metric-sub">
            <DeltaIndicator
              minutes={Math.round((dayMetrics.bedtime - avgDayMetrics.bedtime) * 60)}
              avg={`avg ${fmtTime(avgDayMetrics.bedtime)}`}
            />
          </div>
        </div>
        <div className="metric-card">
          <div className="metric-label">Wake time</div>
          <div className="metric-value">{fmtTime(dayMetrics.wakeTime)}</div>
          <div className="metric-sub">
            <DeltaIndicator
              minutes={Math.round((dayMetrics.wakeTime - avgDayMetrics.wakeTime) * 60)}
              avg={`avg ${fmtTime(avgDayMetrics.wakeTime)}`}
            />
          </div>
        </div>
        <div className="metric-card">
          <div className="metric-label">Awakenings</div>
          <div className="metric-value">{dayMetrics.awakenings}</div>
          <div className="metric-sub">
            <DeltaIndicator
              minutes={Math.round(dayMetrics.awakenings - avgDayMetrics.awakenings)}
              avg={`avg ${avgDayMetrics.awakenings.toFixed(1)}/night`}
            />
          </div>
        </div>
      </div>
      ...
      <div className="temporal-main-row">
        <div className="card timeline-card">
          <div className="card-title">Daily activity timeline</div>
          <div className="card-subtitle">{dayRow.label}, {fmtDayDate(selectedDate)} · {dayWeekLabel}</div>
          <div className="legend">
            {Object.values(ACTIVITY_TYPES).map((a) => (
              <span key={a.label}><span className="legend-dot" style={{ background: a.color }} />{a.label}</span>
            ))}
          </div>
          <WeeklyTimelineChart weekData={[dayRow]} selected={selectedEvent} onSelect={handleSelect} />
        </div>
        ...
        <div className="temporal-side-col">
          ...
          <div className="card summary-card">
            <div className="card-title">Day summary - {dayRow.label}</div>
            <ul className="summary-list">
              <li><span>Sleep</span><span>{fmtDuration(dayActivityTotals.sleep)}</span></li>
              <li><span>Meal preparation</span><span>{fmtDuration(dayActivityTotals.meals)}</span></li>
              <li><span>Outings</span><span>{fmtDuration(dayActivityTotals.outings)}</span></li>
              <li><span>Bathroom</span><span>{fmtDuration(dayActivityTotals.bathroom)}</span></li>
              <li><span>Transitions</span><span>{transitionsForDate(selectedDate)}</span></li>
            </ul>
          </div>
          ...
          <div className="card anomalies-card">
            ...
            {dayAnomalies.length > 0 ? (
              ...
              <div className="event-detail-placeholder">No anomalies for this day</div>
            )}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="dash temporal-dash">
      <TopBar
        ...
        dateRange={activeTab === 'Day view' ? undefined : 'Nov 2016 – Dec 2017'}
      ...
      {activeTab === 'Weekly view' && <WeeklyView />}
      {activeTab === 'Day view' && <DayView />}
      {activeTab !== 'Weekly view' && activeTab !== 'Day view' && (
